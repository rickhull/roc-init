...
By the way, if you want help, **the** best place to get it is [Roc Zulip](https://roc.zulipchat.com/) - we've already had plenty of posts in [the `#advent of code` channel](https://roc.zulipchat.com/#narrow/channel/358903-advent-of-code/) and you're welcome to ask any questions you have there or in [#beginners](https://roc.zulipchat.com/#narrow/channel/231634-beginners/).
...
Here, `WasEmpty` is a tag that isn't wrapping anything. The `List.first` method is just returning it inside the `Err`
...
Here we have an extra `Err` branch, because `List.first` can return `Err(WasEmpty)` if the list was empty,
...
`Err(WasEmpty) => 0`, we would have been silently handling the `Err(BadNumStr)` case using the same logic. That might
not have been what we wanted! By writing out `Err(WasEmpty)` instead of `_ =>`, the compiler would let us know if we
...
(like `Err(WasEmpty) =>` earlier) because it wants to be flexible. If it matched a more restrictive pattern, like
`Err(WasEmpty) =>`, then you couldn't use `.ok_or` with `I64.from_str` because it returns `Err(BadNumStr)`.
...
As we saw with `WasEmpty` and `BadNumStr`, Tags don't have to wrap anything. You can also use them as enumerations, like so:
